...
exports.createCompileMiddleware = createCompileMiddleware;
...
function createBabelCompilers(cfg) {
  ...
  const compileModern = [_constants.compatibilityModes.MODERN, _constants.compatibilityModes.ALL].includes(cfg.compatibilityMode);
  const compileDefault = compileModern || cfg.customBabelConfig || cfg.readUserBabelConfig;
  const compileLegacy = cfg.compatibilityMode === _constants.compatibilityModes.ALL;
  const defaultCompiler = compileDefault ? (0, _babelCompiler.default)({ ...cfg,
    modern: compileModern,
    legacy: false
  }) : null;
  const legacy = compileLegacy ? (0, _babelCompiler.default)({ ...cfg,
    modern: false,
    legacy: true
  }) : null;
  return {
...
function createCompileMiddleware(cfg) {
  ...
  async function compileMiddleware(ctx, next) {
    ...
    if ((0, _utils.isPolyfill)(ctx.url) || !fileExtensions.some(ext => baseURL.endsWith(ext))) {
      return next();
    }
    ...
    const filePath = (0, _utils.getRequestFilePath)(ctx, cfg.rootDir); // we use the legacy compiler if the request ends with a legacy paramter
    ...
    try {
      ...
      let code = await (0, _utils.getBodyAsString)(ctx); // do the default babel compilation, if configured
      ...
      if (cfg.nodeResolve) {
        code = await (0, _resolveModuleImports.resolveModuleImports)(cfg.rootDir, filePath, code, {
          fileExtensions,
